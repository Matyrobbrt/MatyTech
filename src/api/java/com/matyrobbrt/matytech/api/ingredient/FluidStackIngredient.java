package com.matyrobbrt.matytech.api.ingredient;
...
import com.matyrobbrt.matytech.api.util.TagResolver;
...
/**
 * Slightly modified version of Mekanism's FluidStackIngredient
 * (https://github.com/mekanism/Mekanism/blob/1.16.x/src/api/java/mekanism/api/recipes/inputs/FluidStackIngredient.java)
 * 
 * The license header on this file is cancelled, Aidan C. Brady being the
 * copyright holder, with the MIT License
 *
 */
public abstract class FluidStackIngredient implements Predicate<FluidStack> {
	 ...
	 * @param amount   Amount needed.
	 */
	public static FluidStackIngredient from(@Nonnull FluidStack stack, int amount) {
		return from(stack);
	}
	...
	public static FluidStackIngredient deserialize(@Nullable JsonElement json) {
		if (json == null || json.isJsonNull()) { throw new JsonSyntaxException("Ingredient cannot be null"); }
		if (json.isJsonArray()) {
			JsonArray jsonArray = json.getAsJsonArray();
			int size = jsonArray.size();
			if (size == 0) {
				throw new JsonSyntaxException(
						"Ingredient array cannot be empty, at least one ingredient must be defined");
			} else if (size > 1) {
		...
		if (!json.isJsonObject()) { throw new JsonSyntaxException("Expected fluid to be object or array of objects"); }
		...
		if (jsonObject.has("fluid") && jsonObject.has("tag")) {
			...
			return from(Serializers.deserializeFluid(jsonObject));
			...
			if (tag == null) { throw new JsonSyntaxException("Unknown fluid tag '" + resourceLocation + "'"); }
	...
	/**
	 * Combines multiple Fluid Stack Ingredients into a single Fluid Stack
	 * Ingredient.
	 *
	...
	public static FluidStackIngredient createMulti(FluidStackIngredient... ingredients) {
		...
		} else if (ingredients.length == 1) { return ingredients[0]; }
	...
	public abstract boolean testType(@Nonnull FluidStack stack);

	public abstract List<? extends @Nonnull FluidStack> getRepresentations();

	public abstract JsonElement serialize();

	public abstract FluidStack getMatchingInstance(FluidStack stack);

	public abstract long getNeededAmount(FluidStack stack);
	...
	public static class Single extends FluidStackIngredient {
		...
		@Override
		@Nonnull
		public List<@Nonnull FluidStack> getRepresentations() { return Collections.singletonList(fluidInstance); }

		/**
		 * For use in recipe input caching. Do not use this to modify the backing stack.
		 */
		public FluidStack getInputRaw() { return fluidInstance; }
	...
	public static class Tagged extends FluidStackIngredient {
		...
		public List<@Nonnull FluidStack> getRepresentations() {
			...
			for (Fluid fluid : TagResolver.getRepresentations(tag)) {
		 ...
		 */
		public List<Fluid> getRawInput() { return tag.getValues(); }
		...
		public JsonElement serialize() {
			...
			json.addProperty("tag", TagCollectionManager.getInstance().getFluids().getIdOrThrow(tag).toString());
			return json;
		}
	}
	...
	public static class Multi extends FluidStackIngredient {
		...
		public FluidStack getMatchingInstance(@Nonnull FluidStack stack) {
			for (FluidStackIngredient ingredient : ingredients) {
				...
				if (!matchingInstance.isEmpty()) { return matchingInstance; }
			}
			return FluidStack.EMPTY;
		}
